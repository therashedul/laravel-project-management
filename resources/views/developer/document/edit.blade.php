...
@section('content')
            ...
            <div class="card">
                <div class="card-header">Edit Document
                    <span class="float-right">
                        <a class="btn btn-primary" href="{{ route('developer.documents') }}">Document</a>
                    </span>
                </div>
                <div class="card-body">

                    {!! Form::model($editdocument, ['route' => ['developer.documents.update', $editdocument->id], 'method' => 'PATCH', 'enctype' => 'multipart/form-data', 'id' => 'upload']) !!}
                        ...
                        <div class="form-group">
                            ...
                            <div id="post_select_image_container" class="post-select-image-container">
                                ...
                                <h2 class="uploaded-file-name">
                                    {{-- {{ $editdocument->extention }} --}}
                                    @if ($editdocument->extention == '.pdf')
                                        <img src="{{ asset('img/' . 'pdf.png') }}" width="150" height="150" loading="lazy"
                                            class="img-responsive">
                                    @elseif ($editdocument->extention == '.docx')
                                        <img src="{{ asset('img/' . 'docx.png') }}" width="150" height="150"
                                            loading="lazy" class="img-responsive">
                                    @elseif ($editdocument->extention == '.xlsx')
                                        <img src="{{ asset('img/' . 'xlsx.png') }}" width="150" height="150"
                                            loading="lazy" class="img-responsive">
                                    @endif
    ...
    <div class="modal fade" id="image_file_manager">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                {{-- {!! Form::open(['method' => 'get', 'route' => 'developer.documents.destroy', 'enctype' => 'multipart/form-data', 'id' => 'myform']) !!} --}}
                <div class="modal-body">
                    <div class="file-manager">
                        <div class="file-manager-left">
                            <form id="dropzoneForm" class="dropzone" enctype="multipart/form-data"
                                action="{{ route('developer.documents.upload') }}">
                                @csrf
    ...
    <script type="text/javascript">
        ...
        function load_images() {
            $.ajax({
                url: "{{ route('developer.documents.fetch') }}",
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '#image_file_manager #btn_img_delete', function() {
            ...
            $.ajax({
                url: "{{ route('developer.documents.delete') }}",
                data: {
                    name: file_name,
                    imageId: file_id
                },
        ...
        $(document).on('input', '#input_search_image', function() {
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('developer.documents.search') }}",
                data: data,
